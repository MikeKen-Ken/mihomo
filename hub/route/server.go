...
func start(cfg *Config) {
	...
	if len(cfg.Addr) > 0 {
		...
		if err != nil {
			log.Errorln("External controller listen failed: %s", err)
			return
		}
		log.Infoln("RESTful API listening address: %s", l.Addr().String())
		...
		if err = server.Serve(l); err != nil {
			log.Errorln("External controller server error: %s", err)
		}
	}
}
...
func startTLS(cfg *Config) {
	...
	if len(cfg.TLSAddr) > 0 {
		...
		if err != nil {
			log.Errorln("External controller TLS listen failed: %s", err)
			return
		}
		...
		log.Infoln("RESTful API TLS listening address: %s", l.Addr().String())
		...
		if cfg.EchKey != "" {
			...
			if err != nil {
				log.Errorln("External controller TLS server error: %s", err)
				return
			}
		}
		server := &http.Server{
	...
	if len(cfg.UnixAddr) > 0 {
		...
		if _, err := os.Stat(dir); os.IsNotExist(err) {
			if err := os.MkdirAll(dir, 0o755); err != nil {
				log.Errorln("External controller Unix listen failed: %s", err)
				return
			}
		}
		...
		log.Infoln("RESTful API Unix listening address: %s", l.Addr().String())
		...
		unixServer = server
		if err = server.Serve(l); err != nil {
				log.Errorln("External controller Unix server error: %s", err)
		}
	}
}
...
func startPipe(cfg *Config) {
	...
	if len(cfg.PipeAddr) > 0 {
		if !strings.HasPrefix(cfg.PipeAddr, "\\\\.\\pipe\\") { // windows namedpipe must start with "\\.\pipe\"
			log.Errorln("External controller pipe listen error: Windows named pipes must start with \"\\\\.\\pipe\\\"")
			return
		}

		l, err := inbound.ListenNamedPipe(cfg.PipeAddr)
		if err != nil {
			log.Errorln("External controller pipe listen failed: %s", err)
			return
		}
		log.Infoln("RESTful API named pipe listening address: %s", l.Addr().String())
		...
		pipeServer = server
		if err = server.Serve(l); err != nil {
			log.Errorln("External controller pipe server error: %s", err)
		}
	}
}
